...
do
	...
	do
		echo "POST $hosted:$((8000 + numCores))/login" | ./vegeta attack -body=./body.json -duration=1m -rate=$rate -header="Content-Type: application/json" > results/hosted-$numCores-thread-$rate-rate.bin;
